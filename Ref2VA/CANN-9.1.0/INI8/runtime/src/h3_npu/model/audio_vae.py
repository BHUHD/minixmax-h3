"""MiniMax-H3 Audio VAE（Comfy 权重兼容）：DAC encoder + BigVGAN decoder。

从 ComfyUI ``comfy/ldm/minimax/audio_vae.py`` 移植，去掉 comfy.ops，
直接加载 ``minimax_h3_audio_vae_fp32.safetensors``。
潜变量形状 [B, 32, 2, T]（40 latent fps），波形 32 kHz 立体声。
"""
...
class UpSample1d(nn.Module):
    ...
    def forward(self, x):
        # 避免 groups=C 的 depthwise conv_transpose：torch_npu 在 CPU 上会触发
        # 「illegal immediate parameter / Xbyak」错误。
        b, c, _ = x.shape
        x = F.pad(x, (self.pad, self.pad), mode="replicate")
        x = x.reshape(b * c, 1, -1)
        x = F.conv_transpose1d(x, self.filter.to(dtype=x.dtype), stride=self.stride).mul_(self.ratio)
...
class LowPassFilter1d(nn.Module):
    ...
    def forward(self, x):
        # 同上：用 [B*C,1,T] + groups=1 代替 groups=C depthwise，兼容 torch_npu CPU 路径。
        b, c, _ = x.shape
        x = F.pad(x, (self.pad_left, self.pad_right), mode="replicate")
        x = x.reshape(b * c, 1, -1)
        y = F.conv1d(x, self.filter.to(dtype=x.dtype), stride=self.stride)
...
class CausalAttention(nn.Module):
    ...
    def forward(self, x):
        B, N, C = x.shape
        bias = torch.cat((self.q_bias, self.zero_k_bias, self.v_bias)).to(device=x.device, dtype=x.dtype)
        qkv = F.linear(x, self.qkv.weight.to(dtype=x.dtype), bias)
...
def load_audio_vae(
    ...
    *,
    ...
    """加载 Comfy 打包的 audio VAE 权重（FP32，默认放 CPU 以省 NPU HBM）。"""
    ...
    if missing or unexpected:
        print(f"[h3_npu] audio_vae missing={missing[:4]} unexpected={unexpected[:4]}", flush=True)
    model.to(device=device, dtype=dtype)
    model.eval()
    print(f"[h3_npu] audio_vae loaded {path} on {device} dtype={dtype}", flush=True)
    return model
